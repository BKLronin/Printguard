...
while(True):
    ...
    if len(bloblist) < 100 and triggered:
        #img = sensor.snapshot()
        for blob in img.find_blobs(thresholds,roi = (0,0,320,120), pixels_threshold=300, area_threshold=500):
            img.draw_rectangle(blob.rect())
            img.draw_cross(blob.cx(), blob.cy())
    ...
    if len(bloblist) == 100 and triggered:
        for blob in img.find_blobs(thresholds,roi = (50,0,270,120), pixels_threshold=300, area_threshold=500, merge= False):
            ...
            if (blob.cx(), blob.cy()) not in bloblist and len(bloblistObj) < 20  and blob.density() > 0.7:
